...
class AnalyzeView(APIView):
    ...
    Dos formas de traer los bytes (decision de Mariana, 13/Ago/2026):

    1. Body JSON: drive_file_id, carpeta (ej. "PLD/<id_contraparte>"),
       ...
       nombre_archivo, mime_type, expected_document_type,
       servicio_solicitante, metadata (opcional), internal_prompt_key
       (opcional). Usa la cuenta de servicio con domain-wide delegation
       (docint/drive.py::fetch_bytes) - explorador de la Unidad compartida
       dentro de la app.

    2. Multipart con campo 'file': para cuando el analista elige el
       archivo con el Google Picker real desde SU PROPIO Drive personal -
       el navegador ya trajo los bytes directo de Google usando el OAuth
       token del propio usuario (scope drive.readonly), nunca pasa por la
       cuenta de servicio (esa no tiene por que tener acceso al Drive
       personal de nadie). Mismos campos de texto que el modo JSON
       (expected_document_type, servicio_solicitante, etc.) pero como
       form-data en vez de JSON.
    """
    ...
    def post(self, request, *args, **kwargs):
        ...
        if uploaded_file is not None:
            ...
            if not drive_file_id or not carpeta or not perm_key:
                return Response(
                    {
                        "detail": "Se requieren 'drive_file_id', 'carpeta' y 'perm_key' "
                        "(o el campo 'file' si el archivo ya se trajo del Drive personal del usuario)."
                    },
                    status=400,
                )
