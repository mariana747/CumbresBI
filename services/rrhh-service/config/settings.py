...
# RRHH_DB_SOCKET_DIR: cuando corre en Cloud Run, la conexion a Cloud SQL NO es
...
# Conexiones -> Conexiones de Cloud SQL). Configura RRHH_DB_SOCKET_DIR con
# esa ruta completa; en local (Docker Compose) se deja vacio y se usa TCP
# normal via RRHH_DB_HOST/RRHH_DB_PORT como hasta ahora (mismo patron
# que iam-service, ver su config/settings.py).
RRHH_DB_SOCKET_DIR = env("RRHH_DB_SOCKET_DIR", default=None)

if RRHH_DB_SOCKET_DIR:
    _db_host = RRHH_DB_SOCKET_DIR
    _db_port = ""
else:
    _db_host = env("RRHH_DB_HOST", default="rrhh-service-db")
    _db_port = env("RRHH_DB_PORT", default="3306")

DATABASES = {
    "default": {
        "ENGINE": "django.db.backends.mysql",
        "NAME": env("RRHH_DB_NAME", default="rrhh_service"),
        "USER": env("RRHH_DB_USER", default="rrhh_app"),
        "PASSWORD": env("RRHH_DB_PASSWORD", default=""),
        "HOST": _db_host,
        "PORT": _db_port,
