...
# AUDIT_DB_SOCKET_DIR: cuando corre en Cloud Run, la conexion a Cloud SQL NO es
...
# Conexiones -> Conexiones de Cloud SQL). Configura AUDIT_DB_SOCKET_DIR con
# esa ruta completa; en local (Docker Compose) se deja vacio y se usa TCP
# normal via AUDIT_DB_HOST/AUDIT_DB_PORT como hasta ahora (mismo patron
# que iam-service, ver su config/settings.py).
AUDIT_DB_SOCKET_DIR = env("AUDIT_DB_SOCKET_DIR", default=None)

if AUDIT_DB_SOCKET_DIR:
    _db_host = AUDIT_DB_SOCKET_DIR
    _db_port = ""
else:
    _db_host = env("AUDIT_DB_HOST", default="audit-db")
    _db_port = env("AUDIT_DB_PORT", default="3306")

DATABASES = {
    "default": {
        "ENGINE": "django.db.backends.mysql",
        "NAME": env("AUDIT_DB_NAME", default="audit_service"),
        "USER": env("AUDIT_DB_USER", default="audit_app"),
        "PASSWORD": env("AUDIT_DB_PASSWORD", default=""),
        "HOST": _db_host,
        "PORT": _db_port,
