...
import { subirDocumentoTicket, validarTicketDocumento } from "@/lib/tesoreria";
...
export default function TesoreriaDocumentoTicketPage() {
  ...
  useEffect(() => {
    validarTicketDocumento(params.token)
      .then((ticket) => {
        setNombreDocumento(ticket.nombre_documento);
        setIdContrato(ticket.id_contrato);
        setEstado("valido");
      })
      .catch((err) => {
  ...
  async function handleSubir(e: React.FormEvent) {
    ...
    setSubidaError(null);
    try {
      await subirDocumentoTicket({ token: params.token, recaptchaToken, file: archivo });
            ...
            <Stack spacing={2.5}>
              <Stack spacing={2} alignItems="center" textAlign="center">
                ...
                {nombreDocumento && (
                  <Typography variant="body2" color="text.secondary">
                    ...
                    {idContrato && <> del contrato <strong>{idContrato}</strong></>}.
                  </Typography>
                )}
              </Stack>
              ...
              {subidaCompleta ? (
                <Stack spacing={2} alignItems="center" textAlign="center" sx={{ py: 4 }}>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    Tesorería lo va a procesar en breve.
                  </Typography>
                </Stack>
              ) : (
