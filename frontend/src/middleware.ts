...
//
// La cookie de sesion la pone iam-service (otro origen, localhost:8000),
// pero al ser "host-only" para el host "localhost" (sin atributo Domain
// explicito), el navegador la adjunta en cualquier request a "localhost"
// sin importar el puerto - por eso el middleware de localhost:3000 SI
// puede leerla. En produccion, con subdominios reales (app./api.<dominio>)
// esto se resuelve igual via un dominio raiz compartido (ver README.md,
...
export function middleware(request: NextRequest) {
  ...
  const iamBaseUrl = process.env.NEXT_PUBLIC_IAM_API_BASE_URL ?? "http://localhost:8000";
